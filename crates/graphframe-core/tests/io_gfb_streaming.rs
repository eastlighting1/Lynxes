...
use graphframe_core::{
    read_gfb_streaming, read_gfb_streaming_with_options, Direction, EdgeFrame, GfbReadOptions,
    GraphFrame, NodeFrame, COL_EDGE_DIRECTION, COL_EDGE_DST, COL_EDGE_SRC, COL_EDGE_TYPE,
...
fn read_gfb_streaming_with_projection_uses_same_options_contract() {
    ...
    runtime.block_on(async {
        ...
        let batch = poll_fn(|cx| Pin::new(&mut stream).poll_next(cx))
            .await
            .unwrap()
            .unwrap();
        assert_eq!(
