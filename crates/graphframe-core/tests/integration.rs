...
use graphframe_core::{
    parse_gf, read_gfb, read_gfb_with_options, write_gfb, BinaryOp, Direction, EdgeFrame,
    EdgeTypeSpec, Expr, GfbReadOptions, GfbWriteOptions, GraphFrame, NodeFrame, ScalarValue,
...
fn gf_parse_filter_collect_nodes_end_to_end() {
    ...
    assert_eq!(alice.props["age"], graphframe_core::GFValue::Int(25));
...
fn lazy_filter_aggregate_collect_nodes_returns_count_column() {
    let result = demo_graph()
        .lazy()
        .aggregate_neighbors("KNOWS", graphframe_core::AggExpr::Count)
        .collect_nodes()
        .unwrap();
