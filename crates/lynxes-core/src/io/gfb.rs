...
use crate::{EdgeFrame, GFError, GraphFrame, NodeFrame, Result, COL_NODE_LABEL};
...
impl GraphFrame {
    #[cfg(not(target_arch = "wasm32"))]
    pub fn write_gfb<P>(&self, path: P) -> Result<()>
    where
        P: AsRef<Path>,
    {
        self.write_gfb_with_options(path, &GfbWriteOptions::default())
    }

    #[cfg(not(target_arch = "wasm32"))]
    pub fn write_gfb_with_options<P>(&self, path: P, options: &GfbWriteOptions) -> Result<()>
    where
        P: AsRef<Path>,
    {
        write_gfb(self, path, options)
    }

    #[cfg(not(target_arch = "wasm32"))]
    pub fn read_gfb<P>(path: P) -> Result<Self>
    where
        P: AsRef<Path>,
    {
        read_gfb(path)
    ...
    #[cfg(not(target_arch = "wasm32"))]
    pub fn read_gfb_with_options<P>(path: P, options: &GfbReadOptions) -> Result<Self>
    where
        P: AsRef<Path>,
    {
        read_gfb_with_options(path, options)
    }
}
...
mod tests {
    ...
    fn read_gfb_inspect_has_schema_flag_matches_write_options() {
        ...
        graph.write_gfb(&path_no_schema).unwrap();
        graph
            .write_gfb_with_options(
                &path_with_schema,
                &GfbWriteOptions {
