...
use graphframe_core::{
    AqlQuery, ArangoBackend, ArangoConfig, ArangoConnector, BinaryOp, Connector, Direction,
    EdgeFrame, EdgeTypeSpec, Expr, GFError, NodeFrame, ScalarValue, COL_EDGE_DIRECTION,
...
async fn arango_connector_pushes_down_nodes_and_paginates() {
    ...
    assert_eq!(
        ...
        Some(&graphframe_core::AqlValue::String("vertices".to_owned()))
    );
    assert_eq!(
        queries[0].bind_vars.get("offset"),
        Some(&graphframe_core::AqlValue::Int(0))
    );
    assert_eq!(
        queries[1].bind_vars.get("offset"),
        Some(&graphframe_core::AqlValue::Int(2))
    );
}
