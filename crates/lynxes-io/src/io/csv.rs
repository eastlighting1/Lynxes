...
pub fn read_csv_nodes(path: impl AsRef<Path>, options: &CsvNodeReadOptions) -> Result<NodeFrame> {
    ...
    let reader = ReaderBuilder::new(Arc::new(source_schema.clone()))
        .with_format(format)
    ...
    let source_columns = concat_source_columns(&source_schema, &batches)?;
    let rows = source_columns
        .first()
        .map(|array| array.len())
        .unwrap_or_else(|| batches.iter().map(RecordBatch::num_rows).sum());

    let source_names: Vec<&str> = source_schema
        .fields()
        .iter()
        .map(|field| field.name().as_str())
        .collect();

    let id_array = build_id_array(&source_schema, &source_columns, rows, options)?;
    let label_array = build_label_array(&source_schema, &source_columns, rows, options)?;
