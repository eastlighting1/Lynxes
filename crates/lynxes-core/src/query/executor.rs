...
use crate::{
    AggExpr, BinaryOp, Direction, EdgeFrame, EdgeTypeSpec, ExecutionHint, Expr, GFError,
    GraphFrame, LogicalPlan, NodeFrame, Result, ScalarValue, StringOp, UnaryOp, COL_EDGE_DIRECTION,
    COL_EDGE_DST, COL_EDGE_TYPE,
};
            ...
            match direction {
                Direction::Out => {
                    ...
                    for (&n_local, &e_row) in n_locals.iter().zip(e_rows) {
                        ...
                        let Some(candidate) = graph.edge_node_id_by_idx(n_local) else {
                            continue;
                        };
        ...
        match direction {
            Direction::Out => {
                    ...
                    .zip(graph.edges().out_edge_ids(local_idx))
                {
                    ...
                    if let Some(candidate) = graph.edge_node_id_by_idx(n_local) {
                        next.insert(candidate.to_owned());
...
mod tests {
    use super::*;
    use crate::{COL_EDGE_SRC, COL_NODE_ID, COL_NODE_LABEL};
    use arrow_array::{
    ...
    fn scan(source: Arc<GraphFrame>) -> LogicalPlan {
        ...
        impl crate::Connector for DummyConnector {}
    ...
    fn limit_aware_traverse_stops_early() {
        use crate::{Direction as Dir, EdgeTypeSpec, PatternStep};
    ...
    fn partition_parallel_hint_falls_through() {
        use crate::query::logical_plan::PartitionStrategy;
    ...
    fn partition_parallel_expand_matches_serial() {
        ...
        let parallel_plan = LogicalPlan::Hint {
            hint: ExecutionHint::PartitionParallel {
                strategy: crate::query::logical_plan::PartitionStrategy::ExpandFrontier,
            },
            input: Box::new(LogicalPlan::Expand {
                input: Box::new(scan(graph.clone())),
                edge_type: crate::EdgeTypeSpec::Any,
    ...
    fn partition_parallel_expand_respects_edge_type_filter() {
        ...
        let serial_plan = LogicalPlan::Expand {
            input: alice_seed(),
            edge_type: crate::EdgeTypeSpec::Single("KNOWS".to_owned()),
            hops: 2,
            direction: crate::Direction::Out,
            pre_filter: None,
        };
