use graphframe_core::{Connector, Direction, EdgeTypeSpec, Expr, GFError};
...
async fn default_methods_fail_closed_when_unimplemented() {
    ...
    let load_nodes_err = connector
        .load_nodes(
            ...
            None,
            Some(&Expr::Literal {
                value: graphframe_core::ScalarValue::Bool(true),
            }),
            1,
        )
        .await
...
fn crate_graph() -> graphframe_core::GraphFrame {
    let nodes = graphframe_core::NodeFrame::from_record_batch(common::graph_node_batch()).unwrap();
    let edges = graphframe_core::EdgeFrame::from_record_batch(common::graph_edge_batch()).unwrap();
    graphframe_core::GraphFrame::new(nodes, edges).unwrap()
}
