    ...
    pub fn distributed_expand(
        ...
        for _ in 0..hops {
            ...
            for (shard_idx, shard) in self.shards.iter().enumerate() {
                ...
                // Expand via the shard's own lazy API
                let result = shard
                    .lazy()
                    .filter_nodes(build_id_set_filter(&shard_seeds))
                    .expand(edge_type.clone(), 1, direction)
                    .collect()?;
                // Collect newly found nodes
                for id in result.nodes().id_column().iter().flatten() {
                    if visited.insert(id.to_owned()) {
                        next_frontier.insert(id.to_owned());
                    }
                }
                // Track intra-shard edge rows
                let edge_batch = result.edges().to_record_batch();
                // We track via src/dst pairs instead of row numbers since
                // shard.collect() rebuilds rows; just collect node IDs is enough
                // for correctness — edge rows are re-collected in the final pass.
                let _ = edge_batch;
        ...
        let shard_subgraphs: Vec<GraphFrame> = self
            ...
            .map(|shard| {
                let shard_ids: Vec<&str> = visited_set
                    .iter()
                    .copied()
                    .filter(|&id| self.shard_of(id).map_or(false, |s| {
...
///
/// ```no_run
/// use graphframe_core::{GraphPartitioner, GraphPartitionMethod};
///
...
fn build_id_set_filter(ids: &[&str]) -> crate::Expr {
    // OR-chain: _id == ids[0] | _id == ids[1] | ...
    use crate::{BinaryOp, Expr, ScalarValue};
    let mut exprs = ids.iter().map(|&id| Expr::BinaryOp {
        left: Box::new(Expr::Col { name: "_id".to_owned() }),
        op: BinaryOp::Eq,
        right: Box::new(Expr::Literal { value: ScalarValue::String(id.to_owned()) }),
    });
    let first = match exprs.next() {
        Some(e) => e,
        None => Expr::Literal { value: crate::ScalarValue::Bool(false) },
    };
    exprs.fold(first, |acc, e| Expr::Or { left: Box::new(acc), right: Box::new(e) })
}
...
mod tests {
    use super::*;
    use crate::{io::parse_gf, Direction, EdgeTypeSpec};
...
e -[KNOWS]-> f
"#;
        parse_gf(src).unwrap().to_graph_frame().unwrap()
    }
    ...
    fn test_distributed_expand_reachability() {
        ...
        let serial = g
            .lazy()
            .filter_nodes(crate::Expr::BinaryOp {
                left: Box::new(crate::Expr::Col { name: "_id".to_owned() }),
                op: crate::BinaryOp::Eq,
                right: Box::new(crate::Expr::Literal {
                    value: crate::ScalarValue::String("a".to_owned()),
                }),
            })
            .expand(EdgeTypeSpec::Any, 2, Direction::Out)
            .collect()
            .unwrap();
        let serial_ids: HashSet<String> = serial
            .nodes()
            .id_column()
            .iter()
            .flatten()
        ...
        // All serial results must appear in the distributed result
