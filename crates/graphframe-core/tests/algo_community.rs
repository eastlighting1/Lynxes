...
use common::{graph_node_batch, label_field};
...
fn community_detection_returns_uint32_column() {
    ...
    assert_eq!(
        ...
        vec![
            graphframe_core::COL_NODE_ID,
            graphframe_core::COL_NODE_LABEL,
            "community_id"
        ]
    );
    assert_eq!(
...
fn bridge_graph() -> GraphFrame {
    let node_schema = Arc::new(ArrowSchema::new(vec![
        Field::new(graphframe_core::COL_NODE_ID, DataType::Utf8, false),
        label_field(),
    ]));
