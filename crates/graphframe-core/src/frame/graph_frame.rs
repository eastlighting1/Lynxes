...
use crate::{
    query::LazyGraphFrame, EdgeFrame, GFError, NodeFrame, Result, COL_EDGE_DST, COL_EDGE_SRC,
    COL_NODE_LABEL,
};
...
impl GraphFrame {
    ...
    /// Returns a lazy query builder over this graph.
    ///
    /// No execution happens until a later `collect*()` call.
    pub fn lazy(&self) -> LazyGraphFrame {
        LazyGraphFrame::from_graph(self)
    }
